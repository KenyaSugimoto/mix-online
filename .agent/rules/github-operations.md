...
- PR本文に `このPRでできるようになったこと` を必ず記載する（3〜7項目、できないこと/未対応も明記）
- PR本文に `手動動作確認（最低限試してほしいこと）` を必ず記載する（前提、手順、期待値をセットで記載）
- 外部依存やモック制約で再現できない確認項目は、代替確認方法または未確認理由を明記する
...
- テストコードの `describe` / `it` / `test` の description は日本語で記述する
- 既存テストを変更する場合、対象箇所の英語 description は日本語へ統一する
- ユーザーから明示的に別言語指定があった場合のみ、その指定を優先する
- このルールの目的: テスト意図の読み取りコストを下げ、レビュー速度を維持するため
...
- 例外:
  ...
  - 契約値の整合を固定検証する専用テスト
- CIでは `pnpm check:contract-literals` を実行し、違反を検知した場合は失敗させる
- このルールの目的: 契約値変更時の追従漏れ（ハードコード残存）を防ぐため
