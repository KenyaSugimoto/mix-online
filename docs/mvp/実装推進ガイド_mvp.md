...
| M5-12 | ローカル実プレイ成立確認 | HTTP/WS接続経路と進行遷移は成立したが、`table.act` 契約差分（M5-17）解消前のため `BLOCKED` | ローカルで2ユーザーがログイン〜1ハンド完了まで再現できる | [`apps/web/vite.config.ts`](../../apps/web/vite.config.ts), [`apps/web/src/table-store.ts`](../../apps/web/src/table-store.ts), [`E2Eシナリオ集_mvp.md`](./E2Eシナリオ集_mvp.md) |
