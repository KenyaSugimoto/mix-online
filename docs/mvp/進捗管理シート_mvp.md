...
| M2 | ロビー/履歴API実装 | IN_PROGRESS | 33% | Codex | 2026-02-18 | M2-01/M2-02完了、M2-03着手準備 |
...
| --- | --- | --- | --- | --- | --- |
| M2-03 | 認証API基盤（Google callback後のCookie session、`/api/auth/me`、`/api/auth/logout`）を実装 | P0 | IN_PROGRESS | OAuth redirect/Cookie方針が確定している | [`全体アーキテクチャ図_mvp.md`](./全体アーキテクチャ図_mvp.md), [`openapi.yaml`](./openapi.yaml) |
...
| --- | --- | --- | --- | --- |
| 2026-W07 | 初版ドキュメント整備、実装タスク分解（Next/Backlog拡張）、M0-01〜M0-04完了、M1-01〜M1-04完了、M2-01完了、M2-02完了 | M2-03（認証API基盤） | 仕様未決事項（DEC-01）が残存 | M2-03〜M2-06 の順次実装 |
