...
7. ローカル統合確認（M5-12）では Web を `http://localhost:5173`、API/WS を `http://localhost:3000` で起動し、Web 側は Vite proxy（`/api`, `/ws`）経由で接続する。
...
| HP-13 | ハッピーパス  | ハンド終了後のリビール待機フェーズ       |
...
### HP-13 ハンド終了後のリビール待機フェーズ

- 前提条件:
1. 2人以上が `ACTIVE` でハンド進行中。
2. クライアントは配札カード表示を有効化済み。
- 入力（操作）:
1. ハンドを `DealEndEvent` まで進行させる（`SHOWDOWN` / `UNCONTESTED` の双方を確認）。
2. `DealEndEvent` 受信後、次ハンド開始イベントまでの時間を計測する。
3. 待機中のカード表示を確認する。
- 期待値:
1. `DealEndEvent` 直後には `DealInitEvent` が発行されず、数秒の待機が入る。
2. 待機中は直前ハンドのカード表示が保持される。
3. 待機満了後に `SeatStateChangedEvent(reason=NEXT_HAND_ACTIVATE)`（該当席のみ）と次ハンドの `DealInitEvent` が順序どおり発行される。
4. 上記挙動が `SHOWDOWN` / `UNCONTESTED` の双方で成立する。

### M5-12 ローカル統合プレイ確認（手動2ユーザー）
