...
| 対象               | 表示/操作ルール |
| ------------------ | --------------- |
| テーブル画面共通              | 更新ボタン / ログアウトボタン / WS同期状態（`再接続中` など）/ `Seq` 表示 / 自席ステータスパネル / 席操作パネルは表示しない。ログアウトはロビー画面で行う。 |
| 空席                          | 席カードを薄く（破線）表示し、プレイヤー名・スタック・カードは表示しない。未着席ユーザーのみ空席に `着席` ボタンを表示する。 |
| 自席が `ACTIVE` かつ手番       | `table.act` 入力を有効化。タイマー表示を強調。許可アクションのみボタン表示し、許可外ボタンは非表示。 |
| 自席が `ACTIVE` かつ非手番     | アクションボタン無効、進行イベントのみ反映。 |
| 席配置                        | 自席を基準に座席を相対配置し、左隣/右隣が視覚的に左右へ対応するように表示する。 |
...
- 進行ログ欄に `SeatStateChangedEvent.reason/appliesFrom` と `StreetAdvanceEvent.reason` を表示し、遷移理由を画面上で追跡可能にする。
