...
### SCR-02 ロビー（M5-15 追補: 表示名編集）

- ロビー画面に「表示名編集」導線を配置し、`PATCH /api/auth/me/display-name` を呼び出して更新する。
- 編集モードでは `displayName` 入力（maxLength 64）と `保存` / `キャンセル` を表示する。
- 保存中は入力/ボタンを無効化し、成功時は更新後表示名を即時反映する。
- 失敗時は API のエラーメッセージを画面に表示し、再試行可能な状態を維持する。
...
| 対象               | 表示/操作ルール |
| ------------------ | --------------- |
| テーブル画面共通              | 更新ボタン / ログアウトボタン / WS同期状態（`再接続中` など）/ `Seq` 表示 / 自席ステータスパネル / 席操作パネルは表示しない。ログアウトはロビー画面で行う。テーブル表示時はアプリ共通ヘッダー（`Web Client MVP` パネル）を非表示にし、卓領域を優先する。 |
| 空席                          | 席カードを薄く（破線）表示し、プレイヤー名・スタック・カードは表示しない。 |
| 未着席ユーザー                | テーブルアクセス時に着席モーダルを開き、buy-in 入力後に `table.join` を送信する（席番号指定なし、空席へ自動着席）。 |
| 自席が `ACTIVE` かつ手番       | `table.act` 入力を有効化。タイマー表示を強調。許可アクションのみボタン表示し、許可外ボタンは非表示。 |
| 自席が `ACTIVE` かつ非手番     | アクションボタン無効、進行イベントのみ反映。 |
| 席配置                        | Main Pot コンポーネントを中央に置き、その周囲に6席（自席/左右/上）を相対配置する。 |
...
- アクションボタンは卓右側ではなく、卓下部の操作ドックに配置する。
- アクションボタンは横1行で表示し、選択肢が多い場合は横スクロールで収める（2段折返しにしない）。
- 手番強調は「自席固定」ではなく「現在手番席」を強調表示する。
- 直近アクションは操作ドックではなく、各プレイヤーの席パネル付近に表示する。
